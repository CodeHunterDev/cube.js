...
impl AsyncPostgresShim {
    ...
    pub async fn bind(&mut self, body: protocol::Bind) -> Result<(), ConnectionError> {
        ...
        let portal = if let Some(statement) = source_statement {
            ...
            let description = if let Some(description) = plan.to_row_description(format)? {
    ...
    pub async fn process_simple_query(
        ...
        match stmt {
            Statement::Fetch {
                ...
                match self.portals.remove(&(name.to_string())) {
                    None => {
                        trace!("Unable to find portal for cursor: {}. Maybe it was not created.", name.to_string());
                    }
                    Some(portal) => if let Some(mut portal) = portal {
                        self.write_portal(&mut portal, limit).await?;
                        self.portals.insert(name.to_string(), Some(portal));
                    } else {
                        panic!("test");
                    }
                };

                let prepared = if let Some(p) = self.statements.get(&(name.to_string())) {
                    p.as_ref().unwrap()
                } else {
                    return Err(ConnectionError::Protocol(
                        protocol::ErrorResponse::error(
                            protocol::ErrorCode::ProtocolViolation,
                            format!(
                                "Unable to open cursor \"{}\" automatically",
                                name.to_string()
                            ),
                        )
                        .into(),
                ...
                let description = plan.to_row_description(format)?;

                let mut portal = Portal::new(plan, format, description);
            ...
            Statement::Declare {
                ...
                if Some(false) == hold {
                    return Err(ConnectionError::Protocol(
                        protocol::ErrorResponse::error(
                            protocol::ErrorCode::FeatureNotSupported,
                            "HOLD is not supported for DECLARE statement"
                                .to_string(),
                        )
                            .into(),
                    ));
                ...
                self.write_portal(&mut Portal::new(plan, Format::Text, None), 0)
                    .await?;
            }
            ...
            other => {
                ...
                let description = plan.to_row_description(Format::Text)?;

                self.write_portal(&mut Portal::new(plan, Format::Text, description), 0)
                    .await?;
            }
        };
